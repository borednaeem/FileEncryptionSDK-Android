...
@Composable
fun EncryptScreen(viewModel: EncryptScreenViewModel) {
    val screenState by viewModel.screenState.collectAsState(EncryptionScreenState.Loading)
    ...
    Scaffold(
        topBar = {
            AppTopBar(
                stringResource(id = com.example.fileencryptor.core.common.R.string.screen_title_encrypt)
            )
        }
    ) {
        Box(modifier = Modifier.padding(it)) {
            when (screenState) {
                is EncryptionScreenState.Error -> {
                    val errorCode = (screenState as EncryptionScreenState.Error).errorCode
                    ErrorScreen(errorCode) { errorCode ->
                        viewModel.onErrorScreenRetryClicked(errorCode)
                    }
                }

                is EncryptionScreenState.Loaded -> {
                    val loadedState = screenState as EncryptionScreenState.Loaded
                ...
                EncryptionScreenState.Loading -> LoadingScreen()
            }
        }
    }
}
...
fun EncryptScreenContent(
    ...
    Column(
        ...
        ExposedDropdownMenuBox(
            ...
            TextField(
                ...
                modifier = Modifier
                    .fillMaxWidth()
                    .menuAnchor(),
                label = { Text(stringResource(id = com.example.fileencryptor.core.common.R.string.input_label_mode)) },
                trailingIcon = {
                    ExposedDropdownMenuDefaults.TrailingIcon(
                        expanded = modeDropdownMenuBoxExpanded,
                    )
        ...
        Button(
            ...
            Text(text = stringResource(id = com.example.fileencryptor.core.common.R.string.encrypt_confirm_btn))
        }
    }
}
