package com.example.feature.encrypt
...
import com.example.feature.encrypt.model.EncryptionScreenState
...
@HiltViewModel
class EncryptScreenViewModel @Inject constructor(
    private val contentResolver: ContentResolver
) : ViewModel() {

    private val _screenState: MutableStateFlow<EncryptionScreenState> =
        MutableStateFlow(EncryptionScreenState.Loading)
    val screenState: Flow<EncryptionScreenState> = _screenState
                    ...
                    algorithmOptionList = UiOptionList(
                        list = listOf(
                            UiOptionItem(
                                ...
                                com.example.fileencryptor.core.common.R.string.aes
                            ),
                            UiOptionItem(
                                item = EncryptionAlgorithm.Camellia,
                                com.example.fileencryptor.core.common.R.string.camellia
                            ),
                            UiOptionItem(
                                item = EncryptionAlgorithm.Blowfish,
                                com.example.fileencryptor.core.common.R.string.blowfish
                            )
                        ),
                        selectedIndex = 0
                    ),
                    modeOptionList = UiOptionList(
                        list = listOf(
                            UiOptionItem(
                                ...
                                com.example.fileencryptor.core.common.R.string.cbc
                            ),
                            UiOptionItem(
                                item = EncryptionMode.CFB,
                                com.example.fileencryptor.core.common.R.string.cfb
                            ),
                            UiOptionItem(
                                item = EncryptionMode.ECB,
                                com.example.fileencryptor.core.common.R.string.ecb
                            ),
                            UiOptionItem(
                                item = EncryptionMode.OFB,
                                com.example.fileencryptor.core.common.R.string.ofb
                            )
                        ),
                        selectedIndex = null
                    )
    ...
    fun onAlgorithmOptionSelected(encryptionAlgorithm: EncryptionAlgorithm) {
        ...
        if (oldState is EncryptionScreenState.Loaded) {
    ...
    fun onConfirmClicked(context: Context) {
        val stateAfterValidation =
            validateInputs(_screenState.value as EncryptionScreenState.Loaded)

        _screenState.update { stateAfterValidation }

        if (!stateAfterValidation.hasAnyError) {
            _screenState.update { EncryptionScreenState.Loading }
    ...
    private fun validateInputs(uiState: EncryptionScreenState.Loaded): EncryptionScreenState.Loaded {
        var newState: EncryptionScreenState.Loaded = uiState
